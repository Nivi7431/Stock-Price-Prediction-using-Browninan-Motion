...
def predict_stock_prices(ticker_symbol, start_date, days_to_predict):
    ...
    dt = 1
    N = int(T / dt)
    num_simulations = 1000  # Increase to better capture the range
...
class StockPredictionApp:
    def __init__(self, root):
        ...
        self.bg_image = Image.open("D:/Financial Engineering/Fe project/bg.jpg")
        self.bg_image = self.bg_image.resize((self.window_width, self.window_height))
        self.bg_photo = ImageTk.PhotoImage(self.bg_image)

        self.logo_image = Image.open("D:/Financial Engineering/Fe project/logo.png")
    ...
    def show_predicted_graph(self):
        ...
        mape = calculate_mape(actual_prices_df['Close'][-self.days_to_predict:], predicted_prices_df['Close'])
    ...
    def show_bollinger_bands(self):
        ...
        data = get_live_data(ticker_symbol, start_date, end_date)

        data['20MA'] = data['Close'].rolling(window=20).mean()
        data['20STD'] = data['Close'].rolling(window=20).std()
        data['Upper Band'] = data['20MA'] + (data['20STD'] * 2)
        data['Lower Band'] = data['20MA'] - (data['20STD'] * 2)

        fig, ax = plt.subplots()
        data['Close'].plot(ax=ax, label='Close Prices', color='blue')
        data['Upper Band'].plot(ax=ax, label='Upper Band', color='red')
        data['Lower Band'].plot(ax=ax, label='Lower Band', color='green')
        ax.legend()
        ...
        data_with_bollinger = data[['Close', '20MA', 'Upper Band', 'Lower Band']]
        data_with_bollinger.to_excel("predicted_prices_with_chart.xlsx", sheet_name="Predicted Prices with Bollinger Bands", index=True)

        messagebox.showinfo("Bollinger Bands", "Bollinger Bands calculated and saved to Excel.")
